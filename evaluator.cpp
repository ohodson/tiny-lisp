...
ValuePtr Evaluator::eval(ValuePtr expr) {
...
ValuePtr Evaluator::eval_list(ValuePtr expr, Environment& env) {
    ...
    if (first->is_symbol()) {
        ...
        if (symbol == "lambda") {
            ...
            return make_lambda(params, body, std::make_shared<Environment>(env.shared_from_this()));
        }
    }
...
void Evaluator::setup_builtins() {
    ...
    global_env->define("+", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        double sum = 0.0;
        for (auto arg : args) {
            if (!arg->is_number()) {
    ...
    // List operations
    global_env->define("car", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        if (args.size() != 1) {
    ...
    global_env->define("cdr", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        if (args.size() != 1) {
    ...
    global_env->define("cons", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        if (args.size() != 2) {
    ...
    global_env->define("list", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
    ...
    global_env->define("=", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        ...
        if (a->is_number()) {
            return a->as_number() == b->as_number() ? make_symbol("#t") : make_nil();
        }
        
        if (a->is_string()) {
            return a->as_string() == b->as_string() ? make_symbol("#t") : make_nil();
        }
        
        if (a->is_symbol()) {
            return a->as_symbol() == b->as_symbol() ? make_symbol("#t") : make_nil();
        }
    ...
    global_env->define("<", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        ...
        return args[0]->as_number() < args[1]->as_number() ? make_symbol("#t") : make_nil();
    }));
    ...
    // Type predicates
    global_env->define("null?", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        if (args.size() != 1) {
    ...
    global_env->define("number?", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        if (args.size() != 1) {
    ...
    global_env->define("symbol?", make_builtin([](const std::vector<ValuePtr>& args, Environment&) -> ValuePtr {
        if (args.size() != 1) {
