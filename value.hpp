...
using BuiltinFunction = std::function<ValuePtr(const std::vector<ValuePtr>&, Environment&)>;
...
struct Value : public std::enable_shared_from_this<Value> {
    ...
    Value(const std::string& s, ValueType t = ValueType::STRING) : type(t), data(s) {}
    Value(ValuePtr car, ValuePtr cdr) : type(ValueType::CONS), data(std::make_pair(car, cdr)) {}
    ...
    const BuiltinFunction& as_builtin() const { return std::get<BuiltinFunction>(data); }
